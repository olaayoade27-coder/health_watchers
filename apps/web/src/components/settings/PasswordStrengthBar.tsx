...
export default function PasswordStrengthBar({ password }: PasswordStrengthBarProps) {
  ...
  return (
    <div className="mt-2 space-y-1">
      <div className="flex gap-1" role="img" aria-label={`Password strength: ${LABELS[level]}`}>
        {LEVELS.map((_, i) => (
          <div
            key={i}
            className={`h-1.5 flex-1 rounded-full ${i < activeCount ? activeColor : 'bg-gray-200'}`}
          />
        ))}
      </div>
      <p className={`text-xs font-medium ${LABEL_COLORS[level]}`}>{LABELS[level]}</p>
    </div>
  );
}
